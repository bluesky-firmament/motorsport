...
for file in csv_files:
    ...
    if('Race_2' in file):
        race_2_data_list.append(pd.read_csv(file,header=None))
    if('Race_3' in file):
        race_3_data_list.append(pd.read_csv(file,header=None))
#リストを全て行方向に結合
#axis=0:行方向に結合, sort
...
df = pd.concat(race_2_data_list, axis=0, sort=True)
df.to_csv("./Rd" + Rd + "_Race_2_total" + '.csv',index=False)
df = pd.concat(race_3_data_list, axis=0, sort=True)
df.to_csv("./Rd" + Rd + "_Race_3_total" + '.csv',index=False)